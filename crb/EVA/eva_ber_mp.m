...
for SNR_idx = 1:nSNR
    ...
    parfor mc_idx = 1:nMonteCarlo
        ...
        % ---- Generate noise (shared across OTFS variants) ----
        Z_TF_ofdm = (randn(N, M) + 1j*randn(N, M)) * sqrt(N0/2);
        Z_TF_otfs = (randn(N, M) + 1j*randn(N, M)) * sqrt(N0/2);
        Z_DD = sfft(Z_TF_otfs);

        %% ======== OFDM (CSI) — MMSE equalization ========
        Y_TF_ofdm = channel(X, current_path_gains, time_delays, ...
                            current_doppler, deltaf, Ts, 0) + Z_TF_ofdm;
