...
for comSNR = 10:10:30
    ...
    SNRrads = zeros(1,length(rhos));
    rates = zeros(1,length(rhos));
    ...
    for idx = 1:length(rhos)
        ...
        SNRrads(idx) = real(b_opt.' * Drad * conj(b_opt));
        rates(idx) = log2(real(1 + b_opt.' * Dcom * conj(b_opt)));
    end
    
    SNRrads = pow2db(SNRrads);
...
grid minor;
legend("comm SNR = 10 dB", "comm SNR = 20 dB", "comm SNR = 30 dB");
xlabel("Radar SNR (dB)");
